...
string String_to_SHA(string a)
{
	...
	int strLen = strlen(char_array);
	SHA256_CTX ctx;
	unsigned char hash[32];
	string hashStr = "";

	SHA256Init(&ctx);
	SHA256Update(&ctx, (unsigned char*)char_array, strLen);
...
string Object_to_SHA(Block a)
{
	...
	object_string = create_Block_string(a);

	int n = object_string.length();
 
    // declaring character array
    char char_array[n + 1];
 
    // copying the contents of the string to char array
    strcpy(char_array, object_string.c_str());
